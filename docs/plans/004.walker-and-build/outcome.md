...
- Bug fix 2026-05-23 (`## Bug Fixes` step 003): `IndexerResult` and the `index build` / `index rebuild` JSON shape gained a `chunks_chunked` field — chunker output, independent of dry-run. The text summary now reads `indexed <files_chunked> files, <chunks_chunked> chunks` so `--dry-run` is observable from the aggregate line, not just per-file verbose output. Possible impact: `docs/architecture/cli.md` should grow `chunks_chunked` in the pinned JSON shape for `code_index index build` (and the `rebuild` cross-reference) so the architect-facing contract matches the seven-key reality. Phase 7's `cli.md` currently only enumerates six keys for those subcommands.

_Update 2026-05-23: pathspec observation addressed in [`architecture.md`](../../architecture/architecture.md) "Ignore sources"; `chunks_chunked` observation addressed in [`cli.md`](../../architecture/cli.md) "index build" + "index rebuild" + [`embeddings.md`](../../architecture/embeddings.md) "Batching and throughput" + [`config.md`](../../architecture/config.md) schema table._
