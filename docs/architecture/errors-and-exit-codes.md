...
- CLI flag value not in the flag's allowed enum → `cli.bad_enum` (code 1). Producers: `code_index search --mode <bad>` and `code_index usage <bad>`. `detail` includes `flag` (the argument name as it appears in `--help`, e.g. `"--mode"` for a long-flag or `"<topic>"` for a positional argument), `value` (the rejected value), and `expected` (the list of allowed values). Agents reading `detail.flag` should not assume the leading `--` — positional-argument producers (like `usage`) carry the argument's name as displayed by Typer. Distinct from `config.bad_enum` (code 2), which covers TOML enum violations under `[code_index]`.
