...
|---|---|---|
| `init` | Scaffold `docs/.helpers/config.toml` + `.gitignore`. | [init.md](init.md) |
| `index build` | Full build of the SQLite index from the configured roots. | [index-build.md](index-build.md) |
| `index sync` | Incremental update via mtime + size. | [index-sync.md](index-sync.md) |
| `index rebuild` | Drop and rebuild; required after embedding model changes. | [index-rebuild.md](index-rebuild.md) |
| `search` | Hybrid BM25 + dense retrieval over chunks. | [search.md](search.md) |
| `symbols defs\|refs` | Symbol lookup over the stored symbols table. | [symbols.md](symbols.md) |
| `graph callers\|deps` | Edges lookup: who calls X / what does file Y depend on. | [graph.md](graph.md) |
| `config show` | Print resolved config + index meta. Diagnostic; never gates on drift. | [config-show.md](config-show.md) |
